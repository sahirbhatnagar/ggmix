...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in normalizePath(path, winslash = "{}/"{}, mustWork = TRUE): path[1]="{}/home/sahir/git\_repositories/ggmix/"{}: No such file or directory}}\end{kframe}
\end{knitrout}
...
All of our algorithms are implemented in the \texttt{ggmix} R package hosted on GitHub with extensive documentation (\url{https://github.com/greenwoodlab/ggmix}). We provide a brief demonstration of the \texttt{ggmix} package in Appendix~\ref{ap:showcase}.
...
\end{equation}
where $\mathbf{P}\sim \mathcal{N}(0, \eta \sigma^2 \bPhi)$ is the polygenic effect and $\be \sim \mathcal{N}(0, (1-\eta) \sigma^2 \bI)$ is the error term. Here, $\bPhi_{n \times n}$ is the covariance matrix calculated from $\bX^{(kinship)}$, $\bI_{n \times n}$ is the identity matrix and parameters $\sigma^2$ and $\eta \in [0,1]$ determine how the variance is divided between $\mathbf{P}$ and $\be$. The values of the parameters that we used were as follows: narrow sense heritability $\eta=\lbrace 0.1, 0.3 \rbrace$, number of fixed effects $p_{fixed} = 5,000$, number of SNPs used to calculate the kinship matrix $k = 10,000$, percentage of causal SNPs $c=\lbrace 0\%, 1\%\rbrace$ and $\sigma^2 = 1$. In addition to these parameters, we also varied the amount of overlap between the causal SNPs and the SNPs used to generate the kinship matrix. We considered two main scenarios:
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in loadNamespace(name): there is no package called 'ggmix'}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in popkin::plot\_popkin(kinship = list(dat[[1]]\$kinship), mar\_pad = 0.05): object 'dat' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(dat[[1]]\$PC[, 1], dat[[1]]\$PC[, 2], pch = 19, col = rep(RColorBrewer::brewer.pal(10, : object 'dat' not found}}\end{kframe}
\end{knitrout}
...
\begin{equation}\label{eq:correct_sparsity}
A_j = \begin{cases}
1 & \tm{if } (\widehat{\bbeta}_{\hat{\lambda}})_j= (\bbeta)_j = 0\\
1 & \tm{if } (\widehat{\bbeta}_{\hat{\lambda}})_j \neq 0, (\bbeta)_j \neq 0\\
0 & \tm{otherwise}.
\end{cases}
\end{equation} We also compared the test set prediction error based on the refitted unpenalized estimates for each selected model, the estimation error ($||\widehat{\bbeta} - \bbeta||_2^2$), true positive rate ($\sfrac{|\widehat{S}_{\hat{\lambda}} \in  S_0|}{|S_0|}$), false positive rate ($\sfrac{|\widehat{S}_{\hat{\lambda}} \notin S_0|}{|j \notin S_0|}$), and the variance components ($\eta, \sigma^2$) for the polygenic random effect and error term. %The following estimator is used for the error variance of the lasso~\citep{reid2016study}:
%\begin{equation}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in UseMethod("{}separate\_"{}): no applicable method for 'separate\_' applied to an object of class "{}function"{}}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: evaluation nested too deeply: infinite recursion / options(expressions=)?}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'DT' not found}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'DT' not found}}\end{kframe}
\end{knitrout}

In Figure~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship}, we present the results for the scenario with 1\% causal SNPs ($c=0.01$) which were all used in the calculation of the kinship matrix and true heritability $\eta =$10\%. The complete simulation results are shown in supplementary Section~\ref{ap:ggmix_sim_results}. We see that \ggmix ~outperformed both the \texttt{twostep} and \texttt{lasso} in terms of correct sparsity and estimation error (Figure~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship} panels A and B). This was true regardless of true heritability and whether the causal SNPs were included in the calculation of the kinship matrix (Figures~\ref{fig:plot-correct-sparsity-sim-null-model},~\ref{fig:plot-correct-sparsity-sim-1p-causal},~\ref{fig:plot-estimation-error-sim-null-model} and~\ref{fig:plot-estimation-error-sim-1p-causal}). Across all simulation scenarios, \texttt{ggmix} had the smallest root mean squared prediction error (RMSE) on the test set while also producing the most parsimonious models (Figures~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship} panel B,~\ref{fig:plot-mse-nactive-sim-null} and \ref{fig:plot-mse-nactive-sim-1p-causal}). Both the \texttt{lasso} and \texttt{twostep} had on average, slightly higher true positive rate compared to \ggmix ~but came at the cost of a higher false positive rate (Figures~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship} panel D,~\ref{fig:plot-tpr-fpr-sim-null-model} and~\ref{fig:plot-tpr-fpr-sim-1p-causal}). Both the \texttt{twostep} and \ggmix ~overestimated the heritability though \ggmix ~was closer to the true value (Figure~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship} panel E). When none of the causal SNPs were in the kinship, both methods tended to overestimate the truth when $\eta=10\%$ and underestimate when $\eta=30\%$ (Figure~\ref{fig:plot-eta-sim-1p-causal}). Across all simulation scenarios \texttt{ggmix} was able to (on average) correctly estimate the error variance (Figures~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship} panel F, \ref{fig:plot-errorvar-sim-null-model} and~\ref{fig:plot-errorvar-sim-1p-causal}). The \texttt{lasso} tended to overestimate $\sigma^2$ in the null model while the \texttt{twostep} overestimated $\sigma^2$ when none of the causal SNPs were in the kinship matrix. 
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[p\_causal != "{}Null model"{}][structure == "{}block"{}][eta\_p == : object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[p\_causal != "{}Null model"{}][structure == "{}block"{}][eta\_p == : object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(lhs, parent, parent): object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(data = df\_me\_nactive, aes(x = mean.nactive, y = mean.me, : object 'df\_me\_nactive' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(lhs, parent, parent): object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(data = df\_me\_nactive, aes(x = mean.nactive, y = mean.me, : object 'df\_me\_nactive' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[structure == "{}block"{}][p\_causal != "{}Null model"{}][Method \%in\% : object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[structure == "{}block"{}][p\_causal != "{}Null model"{}][eta\_p == : object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(lhs, parent, parent): object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(data = df\_tpr\_fpr, aes(x = mean.fpr, y = mean.tpr, color = Method, : object 'df\_tpr\_fpr' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'pm\_cs' not found}}\end{kframe}
\end{knitrout}
...
Three datasets with contrasting features are used to illustrate the potential advantages of ~\ggmix ~over existing approaches such as PC adjustment in a \texttt{lasso} regression. In the first two datasets, family structure induces low levels of correlation and sparsity in signals. In the last, a dataset involving mouse crosses, correlations are extremely strong and can confound signals.

\subsubsection{UK Biobank}

With more than 500,000 participants, the UK Biobank is one of the largest genotyped health care registry. Among these participants, 147,731 have been inferred to be related to at least one individual in this cohort~\citep{bycroft2018uk}. Such a widespread genetic relatedness can largely confound association studies and bias trait predictions if not propoerly accounted for. Based on 18,150 individuals with ascertained familial relationship (parent-offspring, full siblings, second degree or third degree)~\citep{biobank2015genotyping}, we attempted to derive a polygenic risk score for standing height, using 10,000 SNPs with the largest effect sizes associated with height from a recent large meta-analysis~\citep{yengo2018meta}. We compared the \ggmix-derived polygenic risk score to those derived by the \texttt{twostep} and \texttt{lasso} methods.

We first estimated the pairwise kinship coefficient among the 18,150 reportedly related individuals using KING~\citep{manichaikul2010robust}. We grouped related individuals with a kinship coefficient > 0.044~\citep{manichaikul2010robust} into 8,300 pedigrees. We then randomly split the dataset into a training set, a model selection set and a test set of roughly equal sample size, ensuring all individuals in the same pedigree were assigned into the same set. We inversely normalized the standing height after adjusting for age, sex, genotyping array, assessment center as well as top 10 genetic PCs following Yengo et al~\citep{yengo2018meta}. 

We used \ggmix, \texttt{twostep} and \texttt{lasso} methods respectively to select SNPs being most predictive of the inversely normalized height on the training set. For \ggmix, in addition to supplying a kinship matrix estimated based on approximately 800,000 genome-wide genotyped SNPs, we also tested the feasibility of supplying a kinship matrix estimated using 30,000 randomly selected SNPs. We optimized the choice of $\lambda$ based on prediction RMSE on the model selection set for each method. We finally examined the performance of each derived polygenic risk score on the test set. Similar to ~\ref{simustudy}, we adjusted for the top 10 genetic PCs as unpenalized predictors when fitting the \texttt{lasso} models and removed the kinship effect in the first step of the \texttt{twostep} models using an AIREML algorithm~\citep{gilmour1995average}.

We found that with a kinship matrix estimated using all genotyped SNPs, \ggmix ~had the possibility to achieve a lower RMSE on the model selection set compared to the \texttt{twostep} and \texttt{lasso} methods (Figure~\ref{fig:UKB-Figure}a). An optimized \ggmix-derived polygenic risk score was also able to better predict the trait with lower RMSE and less SNPs on the test set (Figure~\ref{fig:UKB-Figure}b). However, it should be noted that the model estimation became unstable when only 30,000 randomly selected SNPs were adopted to estimate the kinship matrix (Figure~\ref{fig:UKB-Figure}a).
...
However, since GAW20 data consisted of families, \texttt{twostep} methods were used which could have resulted in a large number of false positives or false negatives. \ggmix~is an alternative approach that could be used for selecting the IV while accounting for the family structure of the data.
...
Given that population admixture in the GAW20 data is likely, we estimated the population kinship using REAP~\citep{thornton2012estimating} after decomposing population compositions using ADMIXTURE~\citep{alexander2009fast}. We supplied the estimated kinship matrix directly to \ggmix .
For both the \texttt{lasso} and \texttt{twostep} methods, we adopted the same strategies as described in our simulation study in section ~\ref{simustudy}, supplying the same kinship matrix estimated by REAP.
...
All of the three methods precisely choose the correct predictor without any false positives in more than half of the replicates, given the strong causal signal. When some false positives are selected, \ggmix ~performs comparably to \texttt{twostep}, and the \texttt{lasso} tends to select more false positives (Figure~\ref{fig:GAW20-prediction-RMSE-activeVariable}).
In terms of phenotype prediction, we observed that \ggmix ~outperforms the \texttt{twostep} method without requiring more SNPs, while it achieves roughly the same prediction accuracy as \texttt{lasso} but with fewer non-causal SNPs (Figure~\ref{fig:GAW20-prediction-RMSE-activeVariable}).
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(RMSEACTIVE, aes(x = meanACTIVE, y = meanRMSE, colour = Method, : object 'RMSEACTIVE' not found}}\end{kframe}
\end{knitrout}
...
Similar to the strategy used when analyzing the GAW20 data, we optimized models by tuning the penalty factor such that these two loci are picked up, while the number of other active loci is minimized.
To evaluate robustness of different models, we bootstrapped the 189-sample dataset and repeated the analysis 200 times.
We directly estimated the kinship between mice using genotypes at 625 microsatellite markers. The estimated kinship entered directly into \ggmix ~and \texttt{twostep}. For the \texttt{lasso}, we calculated and included the first 10 principal components of the estimated kinship.
...
We demonstrate that \ggmix ~recognizes the true associations more robustly than \texttt{twostep} and \texttt{lasso}. In almost all (99\%) bootstrap replicates, \ggmix ~is able to capture both true positives, while \texttt{twostep} failed in 19\% of the replicates and \texttt{lasso} failed in 56\% of the replicates by missing of at least one of the two true positives (Figure~\ref{fig:Mice-comparison-fixTPR}).
We also identified several other loci that might also be associated with susceptibility to mycobacterial infection (Table~\ref{tab:Marker}). Among these new potentially-associated markers, D2Mit156 was found to play a role in control of parasite numbers of $Leishmania$ $tropica$ in lymph nodes~\citep{sohrabi2013mapping}. This locus is considered significant by our definition for both \ggmix ~and \texttt{lasso}. An earlier study identified a parent-of-origin effect at D17Mit221 on CD4M levels~\citep{jackson1999multiple}. This effect was more visible in crosses than in parental strains. In addition, D14Mit131, selected only by \ggmix ~, was found to have a 9\% loss of heterozygosity in hybrids of two inbred mouse strains~\citep{c2000allelotype}, indicating the potential presence of putative suppressor genes pertaining to immune surveillance and tumor progression~\citep{lasko1991loss}. This result might also suggest association with anti-bacterial responses yet to be discovered.
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'ggmixfail' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'lassofail' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'twostepfail' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'genotype' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plotGenome\$count <- NA: object 'plotGenome' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in pie(twosteppie, labels = paste0(prop.table(twosteppie) * 100, : object 'twosteppie' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in nrow(plotGenome): object 'plotGenome' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'plotGenome' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'genotype' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plotGenome\$count <- NA: object 'plotGenome' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in pie(lassopie, labels = paste0(prop.table(lassopie) * 100, "{}\%"{}), : object 'lassopie' not found}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in pie(ggmixpie, labels = paste0(prop.table(ggmixpie) * 100, "{}\%"{}), : object 'ggmixpie' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in nrow(plotGenome): object 'plotGenome' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'plotGenome' not found}}\end{kframe}
\end{knitrout}
...
We have developed a general penalized LMM framework called \ggmix ~which simultaneously selects SNPs and adjusts for population structure in high dimensional prediction models. 
Through an extensive simulation study and two real data analyses, we show that the current approaches of PC adjustment and two-stage procedures are not necessarily sufficient to control for confounding by population structure leading to a high number of false positives or false negatives. Furthermore, \ggmix ~showed improved prediction performance with a more parsimonious model compared to both the \texttt{lasso} and \texttt{twostep}. 
...
Many methods for single-SNP analyses avoid this ``proximal contamination''~\citep{lippert2011fast} by using a leave-one-chromosome-out scheme~\citep{loh2015efficient}, i.e., construct the kinship matrix using all chromosomes except the one on which the marker being tested is located. 
However, this approach is not possible if we want to model many SNPs (across many chromosomes) jointly. 
We also demonstrated \ggmix ~using two examples that mimic many experimental designs in genetics. 
In the GAW20 example, we showed that while all methods were able to select the strongest causal SNP, \ggmix ~did so with the least amount of false positives while also maintaining good predictive ability. 
In the mouse crosses example, we showed that \ggmix ~is robust to perturbations in the data using a bootstrap analysis. Indeed, \ggmix ~was able to consistently select the true positives across bootstrap replicates, while \texttt{twostep} failed in 19\% of the replicates and \texttt{lasso} failed in 56\% of the replicates by missing of at least one of the two true positives. Our re-analysis of the data also lead to some potentially new findings, not found by existing methods, that may warrant further study. 
...
\begin{enumerate}
  \item The UK Biobank data is available upon successful project application.
	...
	\item Mouse cross data is available from ES upon request. 
...
\begin{itemize}
	...
	\item[B] \textbf{Additional Simulation Results} - complete simulation results
	\item[C] \textbf{\ggmix ~Package Showcase} - a vignette describing how to use our \ggmix ~R package
\end{itemize}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[p\_causal == "{}Null model"{}][p\_overlap == "{}No causal SNPs in Kinship"{}][structure == : object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_cs' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_esterror' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(data = df\_mse\_nactive, aes(x = mean.nactive, y = mean.me, : object 'df\_mse\_nactive' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_mse\_nactive' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_tpr\_fpr' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_eta' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[structure == "{}block"{}][p\_causal == "{}Null model"{}][p\_overlap == : object 'DT' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_errorvar' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[p\_causal != "{}Null model"{}][structure == "{}block"{}], aes(Method, : object 'DT' not found}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_esterror\_1p' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggplot(DT[structure == "{}block"{}][p\_causal != "{}Null model"{}], aes(Method, : object 'DT' not found}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'p1\_me\_nactive' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in library(ggmix): there is no package called 'ggmix'}}\begin{alltt}
\hlkwd{data}\hlstd{(}\hlstr{"admixed"}\hlstd{)}
\hlkwd{names}\hlstd{(admixed)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'admixed' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot\_popkin(kinship = x, col\_n = coln, mar = xMar, mar\_pad = marPad, : object 'admixed' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ggmix(x = admixed\$x, y = admixed\$y, kinship = admixed\$kin): could not find function "{}ggmix"{}}}\begin{alltt}
\hlkwd{names}\hlstd{(fit)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'fit' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(fit): object 'fit' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in coef(fit, s = c(0.1, 0.02)): object 'fit' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(fit, s = c(0.1, 0.02), newx = admixed\$x[1:5, ]): object 'fit' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in gic(fit): could not find function "{}gic"{}}}\begin{alltt}
\hlkwd{class}\hlstd{(hdbic)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'hdbic' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in gic(fit, an = log(length(admixed\$y))): could not find function "{}gic"{}}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(hdbic): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(bicfit, ylab = "{}BIC"{}): object 'bicfit' not found}}\begin{alltt}
\hlstd{bicfit[[}\hlstr{"lambda.min"}\hlstd{]]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'bicfit' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in coef(hdbic): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in coef(hdbic, type = "{}nonzero"{}): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(hdbic, newx = admixed\$x[1:5, ]): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in ranef(hdbic): could not find function "{}ranef"{}}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(hdbic, type = "{}predicted"{}, newx = admixed\$x, newy = admixed\$y): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(hdbic, type = "{}QQranef"{}, newx = admixed\$x, newy = admixed\$y): object 'hdbic' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(hdbic, type = "{}QQresid"{}, newx = admixed\$x, newy = admixed\$y): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(hdbic, type = "{}Tukey"{}, newx = admixed\$x, newy = admixed\$y): object 'hdbic' not found}}\end{kframe}
\end{knitrout}
