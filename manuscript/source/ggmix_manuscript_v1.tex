...
\caption{Results from 200 replications for the scenario with 1\% causal SNPs ($c=0.01$) which are all used in the calculation of the kinship matrix and true heritability $\eta =10\%$. (A) Correct sparsity as defined by Equation~\eqref{eq:correct_sparsity}. (B) Root mean squared prediction error on the test set for all three methods. For the \texttt{lasso}, the top 10 PCs for test set individuals are calculated by projecting their data onto the training set PC basis. For the \texttt{twostep}, the predicted values from the second step are compared to the observed response. (C) A closer look at the root mean squared prediction error for \texttt{ggmix} and \texttt{lasso} only. (D) True positive vs. false postive rate. (E) Estimated heritability ($\hat{\eta}$). (F) Estimated error variance ($\hat{\sigma^2}$).}\label{fig:plot-6in1-10percentHerit-1pcausal-allinkinship}
\end{figure}
