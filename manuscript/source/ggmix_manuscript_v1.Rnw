...
finalversion <- TRUE # set to FALSE if still in development and you dont want to run the R code
@
...
Methods such GWAS, which test each variant independently, are likely to miss these true associations due to the stringent significance thresholds required to reduce the number of false positives~\citep{manolio2009finding}.
...
motivations include fine mapping, mendel randomization,
