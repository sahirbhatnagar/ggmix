...
We applied \ggmix ~to all 200 GAW20 simulation datasets and also compared its performance to the twostep and lasso methods.
...
Therefore, to avoid ambiguity, we only focused on chromosome 1 where rs9661059 resides.
...
\subsection{Complex Mouse Crosses}
