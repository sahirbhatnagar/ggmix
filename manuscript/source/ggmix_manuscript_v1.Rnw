...
\end {tabular}\par
\bigskip
Significant markers are required to be observed in at least half of the bootstrap replicates in which the corresponding method successfully captures both true positives with a proper penalty factor (Figure~\ref{fig:Mice-comparison-fixTPR}). Since microsatellite markers are not explicitly pointing towards genes, the most adjacent genes are reported without further interpretation. Two true positive controls are omitted.
\end{minipage}
