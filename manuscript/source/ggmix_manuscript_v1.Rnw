...
All of our algorithms are implemented in the \texttt{ggmix} R package hosted on GitHub with extensive documentation (\url{http://sahirbhatnagar.com/ggmix/}). We provide a brief demonstration of the \texttt{ggmix} package in the Appendix.
Our method can also be used for fine-mapping a dense set of SNPs. We can also use it for determining instruments in Mendelian Randomization studies. 
