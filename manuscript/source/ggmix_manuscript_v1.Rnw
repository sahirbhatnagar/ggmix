...
<<plot-kinship-sim, fig.pos = 'H', fig.cap="Example of an empirical kinship matrix used in simulation studies. This scenario models a 1D geography with extensive admixture.", eval=finalversion, cache=TRUE>>=
...
<<plot-pc-sim, fig.pos = 'H', fig.cap="First two principal component scores of the genotype data used to estimate the kinship matrix where each color represents one of the 10 simulated subpopulations.", eval=finalversion, cache=TRUE>>=
@
...
<<simulation-results, eval=TRUE, results = 'hide'>>=
@
...
\begin{itemize}
	...
	\item[B] \textbf{Additional Simulation Results} 
...
<<print-sim-table-for-n-equal-to-k, results='asis'>>=
@
 
 
%\subsection{Null Model $(c = 0)$}

<<plot-correct-sparsity-sim-null-model, dev="cairo_pdf", fig.asp=0.7, fig.width=12, fig.height=9, fig.pos = 'h', fig.cap="Boxplots of the correct sparsity from 200 replications by the true heritability $\\eta = \\lbrace 10\\%, 30\\% \\rbrace$.", eval=FALSE>>=
@

<<plot-estimation-error-sim-null-model, dev="cairo_pdf", fig.asp=0.7, fig.width=12, fig.height=9, fig.pos = 'h', fig.cap="Boxplots of the estimation error from 200 replications by the true heritability $\\eta = \\lbrace 10\\%, 30\\% \\rbrace$.", eval=FALSE>>=
@

<<plot-mse-nactive-sim-null, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Root mean squared prediction error on the test set vs number of active variables from 200 replications by the true heritability $\\eta = \\lbrace 10\\%, 30\\% \\rbrace$.", eval=FALSE>>=
@


<<plot-tpr-fpr-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.heigh=11, fig.pos = 'H', fig.cap="Means $\\pm 1$ standard deviation of true positive rate vs. false positive rate from 200 replications by the true heritability $\\eta = \\lbrace 10\\%, 30\\% \\rbrace$.", eval=FALSE>>=
@


<<plot-eta-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=9, fig.pos = 'H', fig.cap="Boxplots of the heritability estimate $\\hat{\\eta}$ from 200 simulations by the true heritability $\\eta = \\lbrace 10\\%, 30\\% \\rbrace$.", eval=FALSE>>=
@

<<plot-errorvar-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.heigh=11, fig.pos = 'H', fig.cap="Boxplots of the estimated error variance from 200 replications by the true heritability $\\eta = \\lbrace 10\\%, 30\\% \\rbrace$.", eval=FALSE>>=
@


<<plot-runtime-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Run time (in log seconds) for null model for \\texttt{twostep}, \\texttt{lasso} and \\texttt{ggmix}.", eval=FALSE>>=
@
