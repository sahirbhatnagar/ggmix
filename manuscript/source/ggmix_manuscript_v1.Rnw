...
<<plot-kinship-sim, fig.pos = 'H', fig.cap="Example of an empirical kinship matrix used in simulation studies. This scenario models a 1D geography with extensive admixture.", eval=FALSE>>=
...
<<plot-pc-sim, fig.pos = 'H', fig.cap="First two principal component scores of the genotype data used to estimate the kinship matrix where each color represents one of the 3 simulated subpopulations.", eval=FALSE>>=
@
...
<<plot-6in1-10percentHerit-1pcausal-allinkinship, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Results from 200 replications for the scenario with 1\\% causal SNPs ($c=0.01$) which are all used in the calculation of the kinship matrix and true heritability $\\eta =10\\%$. (A) Correct sparsity as defined by Equation~\\eqref{eq:correct_sparsity}. (B) Root mean squared prediction error on the test set for all three methods. For the \\texttt{lasso}, the top 10 PCs for test set individuals are calculated by projecting their data onto the training set PC basis. For the \\texttt{twostep}, the predicted values from the second step are compared to the observed response. (C) A closer look at the root mean squared prediction error for \\texttt{ggmix} and \\texttt{lasso} only. (D) True positive vs. false postive rate. (E) Estimated heritability ($\\hat{\\eta}$). (F) Estimated error variance ($\\hat{\\sigma^2}$).", eval=finalversion>>=
@
...
The \texttt{twostep} and \ggmix ~methods correctly estimate the error variance while the \texttt{lasso} overestimates it for the null model and for when 1\% of the causal SNPs are in the kinship matrix. We see an inflated estimated error variance across all three methods when $c=0.01$ and none of the causal SNPs are in the kinship matrix with the \texttt{lasso} and \ggmix ~performing similarly.
...
\ggmix ~has very good Type 1 error control, while both the \texttt{lasso} and \texttt{twostep} have a very high false positive rate.
