...
On each simulated replicate, we selected the optimal model parameters for each method by fixing true positive rate to 1 while minimizing false positive rate, such that rs9661059 is an active predictor while non-causal SNPs are excluded to the greatest extent.
...
<<GAW20-FalsePositive-fixFPR, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="", eval=finalversion>>=
@

<<GAW20-prediction-RMSE-activeVariable, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="", eval=finalversion>>=
@
