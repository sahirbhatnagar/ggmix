...
\bSigma_{11_{(q\times q)}} & \bSigma_{12} \\
\bSigma_{12_{(N-q)\times q}} & \bSigma_{22_{(N-q)\times (N-q)}}  \\
...
and $\bSigma_{12}$ is the covariance matrix between the training and testing individuals. The phenotype prediction is thus given by:

\begin{align}
\bmu^\star & = \bX^\star \bbeta + \frac{1}{\sigma^2} \bSigma_{12} \bV^{-1} (\bY - \bX \bbeta)\\
