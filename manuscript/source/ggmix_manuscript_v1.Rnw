...
To assess the performance of \texttt{ggmix}, we simulated random genotypes from the BN-PSD admixture model using the \texttt{bnpsd} package~\citep{bnpsd1,bnpsd2}. We used a block diagonal kinship structure with 5 subpopulations. We simulated a single dataset under this block structure and plotted the estimated kinship matrix ($\bPhi$) in the form of a heatmap as shown in Figure~\ref{fig:plot-kinship-sim}. Each block represents a subpopulation, and a darker color indicates a closer genetic relationship. 
...
0 & \tm{if } \tm{else}.
\end{cases}$
We also compare the model error ($\mb{X}\bbeta - \mb{X}\widehat{\bbeta}$), true positive rate, false positive rate, and the variance components for the random effect and error term. The following estimator is used for the error variance of the lasso~\citep{reid2016study}:
\begin{equation}
\frac{1}{n - \hat{s}_{\hat{\lambda}}} \norm{\bY - \bX \widehat{\bbeta}_{\hat{\lambda}}}_2^2.
\end{equation}  
...
<<plot-correct-sparsity-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Boxplots of the correct sparsity from 200 simulations by heritability and number of causal SNPs that were included in the calculation of the kinship matrix for the null model.", eval=finalversion>>=
@

<<plot-correct-sparsity-sim-1p-causal, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Boxplots of the correct sparsity from 200 simulations by heritability and number of causal SNPs that were included in the calculation of the kinship matrix for the model with 1\\% causal SNPs", eval=finalversion>>=
@


<<plot-me-nactive-sim-null, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Model error vs number of active variables results for the null model.", eval=finalversion>>=
@

<<plot-me-nactive-sim-1p-causal, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Model error vs number of active variables results for the model with 1\\% causal SNPs.", eval=finalversion>>=
@
...
<<plot-nactive-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Boxplots of the number of active variables from 200 simulations by kinship geography and number of causal SNPs that were included in the calculation of the kinship matrix.", eval=finalversion>>=
@

<<plot-nactive-sim-1p-causal, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Boxplots of the number of active variables from 200 simulations by kinship geography and number of causal SNPs that were included in the calculation of the kinship matrix.", eval=finalversion>>=
@


<<plot-eta-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Boxplot of $\\hat{\\eta}$ in the \\texttt{ggmix} model from 200 simulations by kinship geography and number of causal SNPs that were included in the calculation of the kinship matrix.", eval=finalversion>>=
@


<<plot-eta-sim-1p-causal, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'H', fig.cap="Boxplot of $\\hat{\\eta}$ in the \\texttt{ggmix} model from 200 simulations by kinship geography and number of causal SNPs that were included in the calculation of the kinship matrix.", eval=finalversion>>=
@


<<plot-errorvar-sim-null-model, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.heigh=11, fig.pos = 'H', fig.cap="Means $\\pm 1$ standard deviation of error variance from 200 simulations by kinship geography and number of causal SNPs that were included in the calculation of the kinship matrix.", eval=finalversion>>=
@

<<plot-errorvar-sim-1p-causal, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.heigh=11, fig.pos = 'H', fig.cap="Means $\\pm 1$ standard deviation of error variance from 200 simulations by kinship geography and number of causal SNPs that were included in the calculation of the kinship matrix.", eval=finalversion>>=
@
