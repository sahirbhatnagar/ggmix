...
In Figure~\ref{fig:plot-6in1-10percentHerit-1pcausal-allinkinship}, we present the results for the scenario with 1\% causal SNPs ($c=0.01$) which are all used in the calculation of the kinship matrix and true heritability $\eta =$10\%  The complete simulation results are shown in supplementary Section~\ref{ap:ggmix_sim_results}. In Figure~\ref{fig:plot-correct-sparsity-sim-1p-causal} we see that \ggmix outperforms both the twostep and lasso in terms of correct sparsity regardless of true heritability and whether the causal SNPs were included in the calculation of the kinship matrix. %methods sometimes estimate a model with a large number of false positives. When the true model contains some causal SNPs, \ggmix ~again outperforms the other two methods in terms of correct sparsity. The distribution of $\widehat{S}_{\hat{\lambda}}$ for each of the three methods is shown in Figure~\ref{fig:plot-nactive-sim-null-model} for $c=0$ and Figure~\ref{fig:plot-nactive-sim-1p-causal} for $c=0.01$ of Supplemental Section~\ref{ap:ggmix_sim_results}.

<<plot-6in1-10percentHerit-1pcausal-allinkinship, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=11, fig.pos = 'h', fig.cap="Results from 200 replications for the scenario with 1\\% causal SNPs ($c=0.01$) which are all used in the calculation of the kinship matrix and true heritability $\eta = 10\\%$. (A) Correct sparsity, (B) Root mean squared prediction error on the test set, (C) Root mean squared prediction error for \texttt{ggmix} and lasso only, (D) True positive vs. false postive rate, (E) Estimated heritability, (F) Estimated error variance.", eval=finalversion>>=
@
