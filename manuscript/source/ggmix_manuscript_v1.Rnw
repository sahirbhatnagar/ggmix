...
<<UKB-Figure, dev="cairo_pdf", fig.asp=0.8, fig.width=12, fig.height=13, fig.pos = 'H', fig.cap="Model selection and testing in the UK Biobank. (a) Root-mean-square error of three methods on the model selection set with respect to a grid search of penalty factor used on the training set. (b) Performance of four methods on the test set with penalty factor optimized on the model selection set. The x-axis has a logarithmic scale. The BSLMM method optimized coefficients of each SNP through an MCMC process on the training set and was directly evaluated on the test set.", eval=finalversion>>=
@
