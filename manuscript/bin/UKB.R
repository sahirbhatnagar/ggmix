...
p1 <- ggplot(valstats, aes(x = index, y = RMSE, color = method)) +
  ...
  theme(
    legend.position = "bottom", axis.text = element_text(size = 15),
...
p2 <- ggplot(teststats, aes(x = log2(coef), y = RMSE, color = method)) +
  ylim(0.9, 1) +
  xlim(9, 12) +
  geom_point(size = 5) +
  xlab(expression(log[2](Number ~ of ~ active ~ variables))) +
...
ukb_long <- do.call(rbind, lapply(1:nrow(ukb_df), function(i) {
  data.frame(
    Chromosome = rep(ukb_df$Chromosome[i], each = ukb_df$freq[i]),
